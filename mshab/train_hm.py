...
# 从自定义模块导入
from mshab.agents.hm import Agent  # 策略代理
from mshab.agents.hm.utils import IterationBasedBatchSampler, worker_init_fn, collate_fn, process_simulation_batch  # 数据采样工具
from mshab.agents.hm.utils import _get_merged_pc_vectorized
...
# HM数据集类（继承自可关闭数据集）
class HMDataset(ClosableDataset):
    def __init__(
        self,
        data_path,
        obs_horizon,
        ...
        for traj_idx in range(num_traj):
            ...
            self.slices += [
                ...
                for start in range(-obs_horizon + 1, L - obs_horizon + 1)
            ]
    ...
    def __getitem__(self, index):
        ...
        for k, v in obs_traj.items():
            if k == "pointcloud":
                obs_seq[k] = v[end-1].unsqueeze(0)  # 只要最后一帧点云
            elif k == "state":
                obs_seq[k] = v[
                    max(0, start) : end
                ] # (L, state_dim)
                ...
                # 处理图像数据格式
                obs_seq[k] = to_tensor(obs_seq[k]).permute(0, 3, 1, 2)  # FS, D, H, W 
            # 在轨迹开始前填充
            if start < 0 and k == "state":
...
def train(cfg: TrainConfig):
    ...
    # 创建数据采样器
    sampler = RandomSampler(hm_dataset, replacement=False)
    ...
    for iteration, data_batch in tqdm(
        ...
        # 准备观测数据
        cloud_coords = data_batch["pcd_coords"]
        cloud_feats = data_batch["pcd_feats"]
            ...
            if check_freq(cfg.algo.eval_freq):
                with torch.no_grad():
                    ...
                    while len(eval_envs.return_queue) < cfg.algo.eval_episodes:
                        ...
                        obs = process_simulation_batch(obs)
                        cloud_coords = obs["pcd_coords"]
                        cloud_feats = obs["pcd_feats"]
                        state = obs["state"]
                        imgs = obs['images']
                        cloud_data = ME.SparseTensor(cloud_feats, cloud_coords)
                        # 获取动作序列
                        action_seq = agent.get_action(cloud_data, imgs, state)  # (B, pred_horizon, act_dim)
                        # 执行动作序列
