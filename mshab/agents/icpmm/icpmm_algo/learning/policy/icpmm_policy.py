...
from icpmm_algo.learning.nn.features import PointNet, ResNet, EnhancedFusion  # PointNet、ResNet
...
class ICPMMPolicy(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # 目标物体位姿解码器
        target_obj_pose_decoder_mlp_hidden_depth: int, # 目标物体位姿解码器MLP隐藏层深度
        target_obj_pose_decoder_mlp_hidden_dim: int, # 目标物体位姿解码器MLP隐藏层维度
        target_obj_pose_dim: int, # target_obj_pose_dim维度
        
        # 观测相关参数
        num_latest_obs: int,  # 使用的最近观测数量(时间步长)
        learnable_intention_readout_token: bool,  # 意图读出令牌是否可学习
        learnable_target_obj_pose_readout_token: bool,  # 目标物体位置读出令牌是否可学习
        
        # ====== Transformer 参数 ======
        tf1_n_embd: int,  # 意图推理Transformer嵌入维度
        tf1_n_layer: int,  # 意图推理Transformer层数
        tf1_n_head: int,  # 意图推理Transformer头数
        tf1_dropout_rate: float,  # 意图推理Transformer dropout率
        tf1_use_geglu: bool,  # 是否使用GEGLU激活函数
        tf2_n_embd: int,  # 动作特征Transformer嵌入维度
        tf2_n_layer: int,  # 动作特征Transformer层数
        tf2_n_head: int,  # 动作特征Transformer头数
        tf2_dropout_rate: float,  # 动作特征Transformer dropout率
        tf2_use_geglu: bool,  # 是否使用GEGLU激活函数
        ...
        self.enhanced_feature_fusion = EnhancedFusion(tf1_n_embd, num_head=8, is_causal=True)

        # 图像键
        self.image_key = "images"
        # resnet
        self.resnet = ResNet(
            output_dim=tf2_n_embd, 
        ...
        if learnable_intention_readout_token:
            ...
            self.intention_readout_token = torch.zeros(tf1_n_embd)
        ...
        if learnable_action_readout_token:
            ...
            self.action_readout_token = nn.Parameter(
                torch.zeros(
                    tf2_n_embd,  # 维度与Transformer嵌入一致
                )
            )
        else:
            # 固定零张量
            self.action_readout_token = torch.zeros(tf2_n_embd)

        # 初始化目标物体位置读出令牌
        if learnable_target_obj_pose_readout_token:
            # 可学习的参数
            self.target_obj_pose_readout_token = nn.Parameter(
                torch.zeros(
                    tf2_n_embd,  # 维度与Transformer嵌入一致
                )
            )
            ...
            # 固定零张量
            self.target_obj_pose_readout_token = torch.zeros(tf2_n_embd)

        # 目标物体位置解码器
        self.target_obj_pose_decoder =  MLP(
            input_dim=tf2_n_embd,  # 输入维度
            hidden_dim=target_obj_pose_decoder_mlp_hidden_dim,  # 隐藏层维度
            output_dim=target_obj_pose_dim,  # 输出维度
            hidden_depth=target_obj_pose_decoder_mlp_hidden_depth,  # 隐藏层深度
            add_output_activation=True,  # 添加输出激活函数
        )
            
        # 初始化意图推理Transformer模型
        self.intention_transformer = GPT(
            n_embd=tf1_n_embd,  # 嵌入维度
            n_layer=tf1_n_layer,  # 层数
            n_head=tf1_n_head,  # 头数
            dropout=tf1_dropout_rate,  # dropout率
            use_geglu=tf1_use_geglu,  # 是否使用GEGLU
        )
        # 初始化动作特征Transformer模型
        self.action_transformer = BiGPT(
            n_embd=tf2_n_embd,  # 嵌入维度
            n_layer=tf2_n_layer,  # 层数
            n_head=tf2_n_head,  # 头数
            dropout=tf2_dropout_rate,  # dropout率
            use_geglu=tf2_use_geglu,  # 是否使用GEGLU
        )
        
        # 初始化全身UNet扩散头用于动作解码
        self.action_decoder = WholeBodyUNetDiffusionHead(
            decoder_part_keys=action_keys,  # 动作解码部位键名
            action_dim_per_part=action_key_dims,  # 各部件动作维度
            intent_dim=tf1_n_embd,  # 意图特征维度
            action_feature_dim=tf2_n_embd,  # 动作特征维度
    ...
    def forward(
        ...
        self.target_obj_pose_readout_token = self.target_obj_pose_readout_token.to(agent_state_feature.device)
        ...
        enhanced_fusion_feature = self.enhanced_feature_fusion(agent_state_feature, pointcloud_feature) # (B, L, E)

        intention_transformer_input = torch.cat(
            [enhanced_fusion_feature, 
             self.intention_readout_token.view(1, 1, -1).expand(enhanced_fusion_feature.shape[0], 1, -1)], 
             dim=1
        )  # (B, L+1, E)
        seq_len = intention_transformer_input.shape[1]  # L+1
        # 位置编码
        position_ids = torch.arange(
            seq_len,  # 序列长度L+1
            device=intention_transformer_input.device,
        ).unsqueeze(0).expand(intention_transformer_input.shape[0], -1)  # (B, L+1)
        # 前向传播
        ...
        ).transpose(0, 1)  # (B, L+1, E)

        intention_token = intention_transformer_output[:, -1, :]  # (B, E)
        intention_feature = intention_token.unsqueeze(1)  # (B, 1, E)
        ...
        action_transformer_input = torch.cat(
            [intention_feature, 
             state_feature, 
             image_feature, 
             self.target_obj_pose_readout_token.view(1, 1, -1).expand(intention_feature.shape[0], 1, -1),
             self.action_readout_token.view(1, 1, -1).expand(intention_feature.shape[0], 1, -1)], 
             dim=1
        )  # (B, 5, E)
        seq_len = action_transformer_input.shape[1]
        ...
        # 设置查询令牌的注意力范围：可以关注所有索引 <= 自身的位置，而之前的Token无法关注查询令牌，只能相互关注
        ...
        position_ids = torch.arange(
            seq_len, 
            device=action_transformer_input.device,
        ).unsqueeze(0).expand(action_transformer_input.shape[0], -1)  # (B, 5)
        # 前向传播
        ...
        target_obj_pose_feature = action_transformer_output[:, -2, :].unsqueeze(1)  # (B, 1, E)
        # 验证输出形状
        ...
        assert target_obj_pose_feature.shape == action_readout_feature.shape

        return intention_feature, action_readout_feature, target_obj_pose_feature  # (B, 1, E), (B, 1, E), (B, 1, E)
    ...
    def compute_loss(
        ...
        *,
        ...
        intention_feature, action_readout_feature, target_obj_pose_feature = self.forward(obs, is_training=True)

        target_obj_pose_pred = self.target_obj_pose_decoder(target_obj_pose_feature.squeeze(1))
        target_obj_pose_pred_loss = torch.nn.functional.mse_loss(target_obj_pose_pred, obs["target_obj_pose"], reduction="none")
        ...
        # 计算扩散损失
        action_pred_loss = self.action_decoder.compute_loss(
        ...
        return action_pred_loss.mean() + target_obj_pose_pred_loss.mean()
    ...
    def inference(
        ...
        *,
        ...
        intention_feature, action_readout_feature, target_obj_pose_feature = self.forward(obs, is_training=False)
