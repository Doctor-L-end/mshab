...
class BiGPT(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        custom_mask: (B, L_tgt) or (B, 1, L_tgt) or (B, L_src, L_tgt)
            because of self-attention, L_tgt = L_src = L
            如果为None，使用默认的因果注意力
            如果提供，将覆盖默认的注意力模式
        """
        ...
        if custom_mask is not None:
            if custom_mask.dim() == 2:
                # (B, L) -> (B, 1, L) -> (B, 1, 1, L)
                attention_mask = custom_mask.unsqueeze(1).unsqueeze(2)
            elif custom_mask.dim() == 3:
                if custom_mask.size(1) == 1:
                    # (B, 1, L) -> (B, 1, 1, L)
                    attention_mask = custom_mask.unsqueeze(2)
                else:
                    # (B, L_src, L_tgt) -> (B, 1, L_src, L_tgt)
                    attention_mask = custom_mask.unsqueeze(1)
    ...
    def _check_input(
        ...
        if custom_mask is not None:
            # 支持多种形状的mask
            assert custom_mask.shape in [(B, L), (B, 1, L), (B, L, L)], \
                f"Expect `custom_mask` to have shape of either ({B}, {L}), ({B}, 1, {L}) or ({B}, {L}, {L}), but got {custom_mask.shape}"
...
class OpenAIGPTModel(OpenAIGPTPreTrainedModel):
    ...
    def forward(
        ...
        if attention_mask is not None:
            # 如果已经提供了3D或4D的attention_mask，直接使用
            if attention_mask.dim() == 3:
                # (B, 1, L) -> (B, 1, 1, L)
                attention_mask = attention_mask.unsqueeze(2)
            elif attention_mask.dim() == 4:
                # (B, 1, L_src, L_tgt) 直接使用
                pass
            else:
                raise ValueError(f"Attention mask should be of dimension 3 or 4, but got {attention_mask.dim()}")
...
class Attention(_Attention):
    def _attn(
        ...
        # 修改点：只有当没有提供外部attention_mask时才使用默认的因果mask
        if attention_mask is None:
            ...
            # 使用外部提供的attention_mask
            # 确保attention_mask的形状与注意力权重匹配
            if attention_mask.size() != w.size():
                # 广播attention_mask到与w相同的形状
                attention_mask = attention_mask.expand_as(w)
            w = w + attention_mask
